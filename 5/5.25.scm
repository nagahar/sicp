...
(define eceval
  (make-machine
    ...
    '(
      ...
      (perform
	(op prompt-for-input) (const ";;; LEC-Eval input:"))
      (assign exp (op read))
      ...
      print-result
      (perform
	(op announce-output) (const ";;; LEC-Eval value:"))
      ...
      (branch (label ev-appl-operand-loop-c))
      (goto (label unknown-procedure-type))
      ;; change
      ;; eval arguments in compound proc
      ev-appl-operand-loop-c
      (save argl)
      (assign exp (op first-operand) (reg unev))
      (test (op last-operand?) (reg unev))
      (branch (label ev-appl-last-arg-c))
      (save env)
      (save unev)
      (assign continue (label ev-appl-accumulate-arg-c))
      ;; change
      (goto (label delay-it))
      ;(goto (label eval-dispatch))

      ev-appl-accumulate-arg-c
      (restore unev)
      (restore env)
      (restore argl)
      (assign argl (op adjoin-arg) (reg val) (reg argl))
      (assign unev (op rest-operands) (reg unev))
      (goto (label ev-appl-operand-loop-c))
      ev-appl-last-arg-c
      ...
      ;; eval arguments in primitive proc
