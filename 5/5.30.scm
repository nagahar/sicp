...
(define (bound? var)
  (and (pair? var) (eq? (car var) 'bound)))

(define (strip-bound-tag var)
  (cdr var))
...
(define eceval-operations
  (list (list 'self-evaluating? self-evaluating?)
	...
	;; change
	(list 'bound? bound?)
	(list 'strip-bound-tag strip-bound-tag)
	))
...
(define eceval
  (make-machine
	...
	(op announce-output) (const ";;; EC-Eval value:"))
      ...
      (branch (label ev-variable-true))
      (goto (label unbound-variable))
      ev-variable-true
      (assign val (op strip-bound-tag) (reg val))
      (goto (reg continue))
      ev-quoted
